...
const powershellScriptCandidates = [
...
function Get-StartMenuParsed {
  $dirs = @("$env:ProgramDataMicrosoftWindowsStart MenuPrograms", "$env:AppDataMicrosoftWindowsStart MenuPrograms")
...
function Find-SystemExe($FileName) {
  ...
  foreach ($path in @("C:WindowsSystem32", "C:WindowsSysWOW64", "$env:SystemRootSystem32", "$env:SystemRootSysWOW64")) {
